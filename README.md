...
so it goes live the moment the app starts. Tokens are stored in the macOS **Keychain** (never the plist);
if a login expires, the bar falls back to the estimate/disk and shows a notice — just log in again.
...
자동 재사용해서 앱 시작 즉시 live로 동작합니다. 토큰은 macOS **키체인**에 저장되며(plist에 안 남김),
만료되면 추정치/디스크로 폴백하고 알림을 띄웁니다 — 다시 로그인만 하면 됩니다.
